...
def probing(args, decoder):
    ...
    # Dataset choosings
    dataset_args = []
    # dataset_args.append(('Google_RE', build_args('Google_RE', args.lowercase)))
    # dataset_args.append(('TREx', build_args('TREx', args.lowercase)))
    # dataset_args.append(('ConceptNet', build_args('ConceptNet', args.lowercase)))
    dataset_args.append(('Squad', build_args('Squad', args.lowercase)))
    ...
    if args.probe_all_layers:
        ...
        data = probe(args, probing_model, tokenizer,
                     dataset_args, vocab, layer=args.probe.probing_layer)

    if args.save_probing_data:
...
def get_probing_model(args, decoder):
    ...
    if args.probe.probing_model == 'BertForMaskedLM':
        probing_model = BertForMaskedLM.from_pretrained(args.bert_model_type)
    elif args.probe.probing_model == 'Decoder' or args.probe.probing_model == 'Huggingface_pretrained_decoder':
...
def probe(args, probing_model, tokenizer, dataset_args, layer, vocab):
    write_to_execution_log(
        100 * '+' + '\t Probing layer: {} \t'.format(layer) + 100 * '+', append_newlines=True)
    ...
    for ele in dataset_args:
        ...
        for relation_args in relation_args_list:
            ...
            args.probe.relation_args = relation_args
            print(
                '---------------- {} ----------------------'.format(args.probe.relation_args.template))
            print(stringify_dotmap(args.probe.relation_args))

            layer_data[ds_name][args.probe.relation_args.relation] = []
            ...
            for step, batch in enumerate(tqdm(dataloader)):
                ...
                for i, prediction_scores in enumerate(batch_prediction_scores):
                    ...
                    metrics_element = calculate_metrics(
                        batch, i, prediction_scores, precision_at_k=args.probe.relation_args.precision_at_k)
            ...
            layer_data[ds_name][args.probe.relation_args.relation].append(
                aggregated_metrics)
            write_metrics(args.run_identifier, ds_name,
                          args.probe.relation_args.relation, args.output_dir, aggregated_metrics)
            write_to_execution_log(ds_name + ': ' + args.probe.relation_args.relation +
                                   '\t' + str(aggregated_metrics), append_newlines=True)

    # Write results to logfile
    if len(google_re_metrices) > 0:
        write_to_execution_log(
            '\n\nGoogle_RE: ' + mean_precisions(google_re_metrices), append_newlines=True)
        layer_data[ds_name]['means'].append(
            mean_precisions(google_re_metrices))
    if len(trex_metrices) > 0:
        write_to_execution_log(
            'Trex: ' + mean_precisions(trex_metrices), append_newlines=True)
        layer_data[ds_name]['means'].append(mean_precisions(trex_metrices))
    write_to_execution_log(220 * '-', append_newlines=True)
...
def probe_all_layers(args, probing_model, tokenizer, dataset_args, vocab):
    ...
    for ele in dataset_args:
        ...
        for n, relation_args in enumerate(relation_args_list):
            ...
            args.probe.relation_args = relation_args
            print('---------------- {} ({}/{}) ----------------------'.format(
                args.probe.relation_args.template, n, len(relation_args_list)))

            data[ds_name][args.probe.relation_args.relation] = []
            ...
            for step, batch in enumerate(tqdm(dataloader)):
                ...
                all_layer_predictions = probing_model(
                    input_ids_batch, masked_lm_labels=input_ids_batch, attention_mask=attention_mask_batch, all_layers=args.probe.all_layers)
            ...
            for layer in range(12):
                ...
                data[ds_name][args.probe.relation_args.relation].append(
                    aggregated_metrics)
                ...
                write_to_execution_log(ds_name + ': ' + args.probe.relation_args.relation +
                                       '\t Layer {}\t\n\n\n\n\n\n\n'.format(layer + 1) + str(aggregated_metrics), append_newlines=True)

    # Write results to logfile
    for layer in range(12):
        if len(google_re_metrices[layer]) > 0:
            write_to_execution_log('\n\nGoogle_RE \t Layer {}: '.format(
                layer + 1) + mean_precisions(google_re_metrices[layer]), append_newlines=True)
        ...
        if len(trex_metrices[layer]) > 0:
            write_to_execution_log('Trex \t Layer {}: '.format(
                layer + 1) + mean_precisions(trex_metrices[layer]), append_newlines=True)
            data[ds_name]['means'].append(
...
def save_probing_data(args, data):
    with open('{}{}_data.json'.format(args.output_base_dir, args.bert_type), 'w') as outfile:
