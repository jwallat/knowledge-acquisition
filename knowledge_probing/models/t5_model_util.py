...
def ssm_tokens(inputs, tokenizer, args):
    ...
    #print(inputs)
    for unit in inputs:
        sample_ids = unit[0].clone()
        sample_ids_clone = sample_ids.clone()
        entities_ids = unit[1].copy()
        others_ids = unit[2].copy()
        pad_ids = torch.where(sample_ids == 0)
        mask_num = int(0.15*(len(sample_ids)-len(pad_ids[0])))
        entities_tokens_length = sum([len(i) for i in entities_ids])
        entities_lst = []
        entities_tokens_num = 0
        if (entities_tokens_length >= int(0.7*mask_num)) and (len(others_ids) >= (mask_num-int(0.7*mask_num))):
            while entities_tokens_num < int(0.7*mask_num):
                random.shuffle(entities_ids)
                entity = entities_ids.pop()
                entities_lst.append(entity)
                entities_tokens_num += len(entity)
            if mask_num > entities_tokens_num:
                random_token_ids = random.sample(others_ids, mask_num-entities_tokens_num)
            else:
                random_token_ids = []
        elif (entities_tokens_length >= int(0.7*mask_num)) and (len(others_ids) < (mask_num-int(0.7*mask_num))):
            random_token_ids = random.sample(others_ids, mask_num - int(0.8*len(others_ids)))
            while entities_tokens_num < (mask_num - len(random_token_ids)):
                random.shuffle(entities_ids)
                entity = entities_ids.pop()
                entities_lst.append(entity)
                entities_tokens_num += len(entity)
        else:
            for ele in entities_ids:
                entities_lst.append(ele)
                entities_tokens_num += len(ele)
            random_token_ids = random.sample(others_ids, mask_num - entities_tokens_num)
        entities_lst = sorted(entities_lst, key=lambda x: x[0])
        entities_lst_copy = entities_lst.copy()
        entities_lst_first = [x[0] for x in entities_lst]
        entities_lst_first.extend(random_token_ids)
        entities_lst_first.sort()
        if entities_lst:
            for i, ele in enumerate(entities_lst_first.copy()):
                if ele in entities_lst[0]:
                    entities_lst_first[i] = entities_lst.pop(0)
                if not entities_lst:
                    break
        t5_label = []
        mask_num = 0
        for ele in entities_lst_first:
            if not isinstance(ele, List):
                try:
                    t5_label.extend([32099-mask_num, int(sample_ids[ele])])
                except:
                    print(t5_label, sample_ids.device)
                    print(entities_lst_first, sample_ids.device)
                    print(mask_num, ele, sample_ids.device)
                    print('ids', sample_ids, sample_ids.device)
                    print('idscopy', sample_ids_clone, sample_ids.device)
                    sys.exit()
            else:
                try:
                    t5_label.extend([32099-mask_num]+[int(sample_ids[x]) for x in ele])
                except:
                    print('!!!!!!!!!!!!')
                    print(t5_label, sample_ids.device)
                    print(entities_lst_first, sample_ids.device)
                    ...
                    sys.exit()
            mask_num += 1
        t5_label.extend([32099-mask_num, 1])
        t5_label = torch.tensor(t5_label, device=sample_ids.device, dtype=sample_ids.dtype)
        t5_labels.append(t5_label)
        replace_idx = []
        delete_idx = []
        for ele in entities_lst_copy:
            if len(ele) == 1:
                replace_idx.extend(ele)
            else:
                replace_idx.append(ele[0])
                delete_idx.extend(ele[1:])
        replace_idx.extend(random_token_ids)
        sample_ids[replace_idx] = -101
        sample_ids[delete_idx] = -99
        sample_ids = sample_ids[torch.where(sample_ids != -99)]
        mask_num = 0
        sample_ids_clone = sample_ids.detach()
        for i, ele in enumerate(sample_ids_clone):
            if ele == -101:
                sample_ids[i] = 32099 - mask_num
                mask_num += 1
        ex = torch.zeros(tokenizer.model_max_length - len(sample_ids), device=sample_ids.device, dtype=sample_ids.dtype)
        sample_ids = torch.cat((sample_ids, ex), 0)
        input_ids.append(sample_ids)
    input_ids = pad_sequence(input_ids, batch_first=True)
    ex = torch.zeros(tokenizer.model_max_length - len(t5_labels[0]), device=t5_labels[0].device, dtype=t5_labels[0].dtype)
    t5_labels[0] = torch.cat((t5_labels[0], ex), 0)
    t5_labels = pad_sequence(t5_labels, batch_first=True)

    return input_ids, t5_labels
