...
class BertDecoder(BaseDecoder):
    ...
    def forward(self, inputs, masked_lm_labels, attention_mask=None, layer=None, batchs=None):
        ...
        #print('c', bert_outputs[2])
        embeddings = bert_outputs[2][layer]
        # Feed embeddings into decoder
        decoder_outputs = self.decoder(embeddings, masked_lm_labels=masked_lm_labels)
        loss = decoder_outputs[0]
        prediction_scores = decoder_outputs[1]
        self.total_num_training_steps = self.total_num_training_steps + 1
        if self.total_num_training_steps% 50 == 0:
            print('Bert_total_num_training_steps: ' , self.total_num_training_steps)
            #print(masked_lm_labels)
            #print(len(masked_lm_labels))
            if batchs == None:
                mask_idx = self.get_index_for_masked_token(masked_lm_labels[0].cpu())  # [MASK]
                print('Ground-truth answer1: ', self.tokenizer.decode([input_ids[0][mask_idx]]))
            else:
                mask_idx = self.get_index_for_masked_token(input_ids[0].cpu())
                print('Ground-truth answer2: ', self.tokenizer.decode([batchs[0][mask_idx]]))

            #print('Ground-truth answer: ', self.tokenizer.decode([input_ids[0][mask_idx]]))

            print('Top predictions for the masked token: ', topk(prediction_scores, mask_idx, k=10, tokenizer=self.tokenizer))

        return loss, prediction_scores

    def probe(self, batch, layer: int, relation_args) -> List:
        #print('batch of probe in bert_decoder: ', batch)

        input_ids_batch = batch['temp_sentences']
        masked_lm_labels = batch['masked_sentences']
        attention_mask_batch = batch['attention_mask']
        input_ids_batch = input_ids_batch.to(self.hparams.device)
        masked_lm_labels = masked_lm_labels.to(self.hparams.device)
        attention_mask_batch = attention_mask_batch.to(self.hparams.device)
        # Get predictions from models
        outputs = self.forward(inputs=input_ids_batch, masked_lm_labels=masked_lm_labels,
        ...
        for i, prediction_scores in enumerate(batch_prediction_scores):
            ...
            metrics_element = calculate_metrics(batch, i, prediction_scores, precision_at_k=relation_args.precision_at_k, tokenizer=self.tokenizer)
            metrics_elements_from_batch.append(metrics_element)
        #print(metrics_elements_from_batch)
    ...
    def set_to_train(self):
        ...
        print('Head_decoder set to train')
