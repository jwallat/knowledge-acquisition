...
def main(args):
    ...
    with open(args.run_name+'.txt', 'w', encoding='utf-8') as f:
        f.write(args.decoder_save_dir)
    if args.do_training:
        if args.save_all:
            args.save_top_k = -1
        decoder = training(args, decoder)
        decoder.hparams.multitask = False
    args = update_args4ft(args=args)
    decoder.hparams.extend4probing = True
    decoder.hparams.probing_batch_size = 32
    if args.do_probing and args.did_multitask:
